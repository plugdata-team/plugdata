...
struct less {
    bool operator()(element_type const& a, element_type const& b) const PLF_NOEXCEPT
    {
        return a < b;
    }
};
...
struct equal_to {
    ...
    explicit equal_to(element_type const& store_value) PLF_NOEXCEPT : value(store_value)
    {
    }

    bool operator()(element_type const& compare_value) const PLF_NOEXCEPT
    {
...
static PLF_CONSTFUNC void* void_cast(source_pointer_type const source_pointer) PLF_NOEXCEPT
{
#    ifdef PLF_CPP20_SUPPORT
...
enum priority { performance = 1,
    memory_use = 4 };

#endif

template<class element_type, class allocator_type = std::allocator<element_type>>
class stack : private allocator_type // Empty base class optimisation - inheriting allocator functions
{
public:
...
private:
    ...
    struct group : private allocator_type {
        element_pointer_type const elements;
        group_pointer_type next_group, previous_group;
        element_pointer_type const end; // One-past the back element
        ...
        group(size_type const elements_per_group, group_pointer_type const previous = NULL) PLF_NOEXCEPT : elements(NULL)
    ...
    struct ebco_pair : group_allocator_type // Packaging the group allocator with the least-used member variable, for empty-base-class optimization
    {
        ...
        ebco_pair(size_type const max_elements, allocator_type const& alloc) PLF_NOEXCEPT : group_allocator_type(alloc)
            , max_block_capacity(max_elements)
        {
        }
    } group_allocator_pair;

    void check_capacities_conformance(size_type const min, size_type const max) const
    {
...
public:
    ...
    // Default constructor:
    PLF_CONSTFUNC stack() PLF_NOEXCEPT_ALLOCATOR : current_group(NULL)
        , first_group(NULL)
        , top_element(NULL)
        , start_element(NULL)
        , end_element(NULL)
    ...
    stack(size_type const min, size_type const max = default_max_block_capacity())
        : current_group(NULL)
        , first_group(NULL)
        , top_element(NULL)
        , start_element(NULL)
    ...
    stack(size_type const min, size_type const max, allocator_type const& alloc)
        : allocator_type(alloc)
        , current_group(NULL)
        , first_group(NULL)
        , top_element(NULL)
...
private:
    void allocate_new_group(size_type const capacity, group_pointer_type const previous_group)
    {
...
#    endif
    ...
    void copy_from_source(stack const& source) // Note: this function is only called on an empty un-initialize()'d stack
    {
        assert(&source != this);
        ...
        group_pointer_type const end_copy_group = source.current_group;

        if (source.total_size <= group_allocator_pair.max_block_capacity) // most common case
        {
            size_type const original_min_block_capacity = min_block_capacity;
...
#endif                                                                          // If compiler doesn't support traits, iterate regardless - trivial destructors will not be called, hopefully compiler will optimise this loop out for POD types
        {
            if (total_size != 0) {
                while (first_group != current_group) {
                    element_pointer_type const past_end = first_group->end;
                    ...
                    group_pointer_type const next_group = first_group->next_group;
                    deallocate_group(first_group);
                    first_group = next_group;
                }

                // Special case for current group:
                element_pointer_type const past_end = top_element + 1;
...
public:
    ...
    stack(stack const& source, allocator_type const& alloc)
        : allocator_type(alloc)
        , current_group(NULL)
        , first_group(NULL)
        , top_element(NULL)
...
#ifdef PLF_MOVE_SEMANTICS_SUPPORT
    // move constructor
    stack(stack&& source) PLF_NOEXCEPT : allocator_type(std::move(static_cast<allocator_type&>(source)))
    ...
    stack(stack&& source, allocator_type const& alloc)
        : allocator_type(alloc)
        , current_group(std::move(source.current_group))
        , first_group(std::move(source.first_group))
        , top_element(std::move(source.top_element))
...
#    endif
    ...
    void pop() // Exception may occur if stack is empty
    {
        ...
        if (--total_size != 0 & top_element-- == start_element) { // ie. is start element, but not first group in stack
            current_group = current_group->previous_group;
            start_element = current_group->elements;
            end_element = current_group->end;
...
private:
    ...
    {
#ifdef PLF_MOVE_SEMANTICS_SUPPORT
        stack temp(min_block_capacity > total_size ? min_block_capacity : total_size > group_allocator_pair.max_block_capacity ? group_allocator_pair.max_block_capacity
                                                                                                                               : total_size.load(),
            group_allocator_pair.max_block_capacity); // Make first allocated block as large as total_size, where possible
...
public:
    void reshape(size_type const min, size_type const max)
    {
        check_capacities_conformance(min, max);
...
#    endif
    ...
    friend bool operator==(stack const& lh, stack const& rh) PLF_NOEXCEPT
    {
    ...
    void trim() PLF_NOEXCEPT
    {
        ...
        while (temp_group != NULL) {
            group_pointer_type const next_group = temp_group->next_group;
    ...
    void reserve(size_type reserve_amount)
    {
        ...
        {
            size_type const original_min_elements = min_block_capacity;
...
#    endif
...
#endif
        {
            ...
            group_pointer_type const swap_current_group = current_group, swap_first_group = first_group;
            element_pointer_type const swap_top_element = top_element, swap_start_element = start_element, swap_end_element = end_element;
            size_type const swap_total_size = total_size, swap_total_capacity = total_capacity, swap_min_block_capacity = min_block_capacity, swap_max_block_capacity = group_allocator_pair.max_block_capacity;
    ...
    public:
        ...
        stack_iterator() PLF_NOEXCEPT : group_pointer(NULL)
            , element_pointer(NULL)
        {
        }

        stack_iterator(stack_iterator const& source) PLF_NOEXCEPT : group_pointer(source.group_pointer)
...
#ifdef PLF_MOVE_SEMANTICS_SUPPORT
        stack_iterator(stack_iterator&& source) PLF_NOEXCEPT : group_pointer(std::move(source.group_pointer))
...
#    endif
        ...
        bool operator==(stack_iterator<!is_const> const& rh) const PLF_NOEXCEPT
        {
            return element_pointer == rh.element_pointer;
        }

        bool operator!=(stack_iterator const& rh) const PLF_NOEXCEPT
        {
            return element_pointer != rh.element_pointer;
        }
        ...
        {
            stack_iterator const copy(*this);
            ++*this;
            return copy;
        }
    ...
    private:
        ...
        stack_iterator(group_pointer_type const group_p, pointer_type const element_p) PLF_NOEXCEPT : group_pointer(group_p)
    ...
    public:
        ...
        stack_reverse_iterator(stack_reverse_iterator const& source) PLF_NOEXCEPT : current(source.current)
        {
        }
        ...
        stack_reverse_iterator(stack_iterator<is_const_r> const& source) PLF_NOEXCEPT : current(source)
        {
            ++*this;
        }
...
#ifdef PLF_MOVE_SEMANTICS_SUPPORT
        stack_reverse_iterator(stack_reverse_iterator&& source) PLF_NOEXCEPT : current(std::move(source.current))
        {
        }
...
#    endif
        ...
        stack_reverse_iterator& operator=(stack_reverse_iterator const& source) PLF_NOEXCEPT
        {
            current = source.current;
            return *this;
        }
...
#    endif
        ...
        bool operator==(stack_reverse_iterator<!is_const_r> const& rh) const PLF_NOEXCEPT
        {
            return current == rh.current;
        }

        bool operator!=(stack_reverse_iterator const& rh) const PLF_NOEXCEPT
        {
            return current != rh.current;
        }
        ...
        {
            stack_reverse_iterator const copy(*this);
            ++*this;
            return copy;
        }
    ...
    private:
        // Used by rend(), etc:
        stack_reverse_iterator(group_pointer_type const group_p, pointer_type const element_p) PLF_NOEXCEPT : current(group_p, element_p) { }
