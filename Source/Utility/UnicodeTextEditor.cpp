...
public:
    UniformTextSection(String const& text, Font const& f, Colour col, juce_wchar passwordCharToUse)
        : font(f)
        , colour(col)
    ...
    UniformTextSection(UniformTextSection const&) = default;
    UniformTextSection(UniformTextSection&&) = default;

    UniformTextSection& operator=(UniformTextSection const&) = delete;
    ...
    void setFont(Font const& newFont, const juce_wchar passwordCharToUse)
    {
...
private:
    void initialiseAtoms(String const& textToParse)
    {
...
struct UnicodeTextEditor::Iterator {
    Iterator(UnicodeTextEditor const& ed)
    ...
    TextAtom const* atom = nullptr;

private:
    OwnedArray<UniformTextSection> const& sections;
    UniformTextSection const* currentSection = nullptr;
    int sectionIndex = 0, atomIndex = 0;
    Justification justification;
    Point<float> const bottomRight;
    float const wordWrapWidth;
    const juce_wchar passwordCharacter;
    float const lineSpacing;
    bool const underlineWhitespace;
    TextAtom longAtom;

    bool chunkLongAtom(bool shouldStartNewLine)
    {
        auto const numRemaining = longAtom.atomText.length() - longAtom.numChars;
        ...
        auto const numChars = jmax(1, split);
    ...
    bool shouldWrap(float const x) const noexcept
    {
...
struct UnicodeTextEditor::InsertAction : public UndoableAction {
    InsertAction(UnicodeTextEditor& ed, String const& newText, int insertPos,
        Font const& newFont, Colour newColour, int oldCaret, int newCaret)
        : owner(ed)
        , text(newText)
...
private:
    ...
    const String text;
    int const insertIndex, oldCaretPos, newCaretPos;
    const Font font;
    const Colour colour;
...
struct UnicodeTextEditor::RemoveAction : public UndoableAction {
    RemoveAction(UnicodeTextEditor& ed, Range<int> rangeToRemove, int oldCaret, int newCaret,
        Array<UniformTextSection*> const& oldSections)
        : owner(ed)
        , range(rangeToRemove)
        , oldCaretPos(oldCaret)
        , newCaretPos(newCaret)
...
private:
    UnicodeTextEditor& owner;
    Range<int> const range;
    int const oldCaretPos, newCaretPos;
...
struct UnicodeTextEditor::TextHolderComponent : public Component
    ...
    TextHolderComponent(UnicodeTextEditor& ed)
        : owner(ed)
    {
...
struct UnicodeTextEditor::TextEditorViewport : public Viewport {
    TextEditorViewport(UnicodeTextEditor& ed)
        : owner(ed)
    {
    }

    void visibleAreaChanged(Rectangle<int> const&) override
    {
...
int const textChangeMessageId = 0x10003001;
int const returnKeyMessageId = 0x10003002;
int const escapeKeyMessageId = 0x10003003;
int const focusLossMessageId = 0x10003004;

int const maxActionsPerTransaction = 100;
...
//==============================================================================
UnicodeTextEditor::UnicodeTextEditor(String const& name, juce_wchar passwordChar)
    : Component(name)
...
bool UnicodeTextEditor::undoOrRedo(bool const shouldUndo)
{
    if (!isReadOnly()) {
        newTransaction();
...
//==============================================================================
void UnicodeTextEditor::setMultiLine(bool const shouldBeMultiLine,
    bool const shouldWordWrap)
{
...
//==============================================================================
void UnicodeTextEditor::setFont(Font const& newFont)
{
    currentFont = newFont;
    scrollToMakeSureCursorIsVisible();
}

void UnicodeTextEditor::applyFontToAllText(Font const& newFont, bool changeCurrentFont)
{
...
void UnicodeTextEditor::applyColourToAllText(Colour const& newColour, bool changeCurrentTextColour)
{
    for (auto* uts : sections)
...
UnicodeTextEditor::LengthAndCharacterRestriction::LengthAndCharacterRestriction(int maxLen, String const& chars)
    : allowedCharacters(chars)
    , maxLength(maxLen)
{
}

String UnicodeTextEditor::LengthAndCharacterRestriction::filterNewText(UnicodeTextEditor& ed, String const& newInput)
{
    String t(newInput);
...
void UnicodeTextEditor::setInputRestrictions(int maxLen, String const& chars)
{
    setInputFilter(new LengthAndCharacterRestriction(maxLen, chars), true);
}

void UnicodeTextEditor::setTextToShowWhenEmpty(String const& text, Colour colourToUse)
{
...
void UnicodeTextEditor::setText(String const& newText, bool sendTextChangeMessage)
{
...
void UnicodeTextEditor::setCaretPosition(int const newIndex)
{
...
void UnicodeTextEditor::scrollEditorToPositionCaret(int const desiredCaretX,
    int const desiredCaretY)
...
void UnicodeTextEditor::checkLayout()
{
    if (getWordWrapWidth() > 0) {
        auto const textBottom = Iterator(*this).getTotalTextHeight() + topIndent;
        auto const textRight = jmax(viewport->getMaximumVisibleWidth(),
...
void UnicodeTextEditor::setScrollToShowCursor(bool const shouldScrollToShowCursor)
{
...
void UnicodeTextEditor::moveCaretTo(int const newPosition, bool const isSelecting)
{
    if (isSelecting) {
...
int UnicodeTextEditor::getTextIndexAt(int const x, int const y) const
{
    auto const offset = getTextOffset();

    return indexAtPosition((float)(x - offset.x),
        (float)(y - offset.y));
}

int UnicodeTextEditor::getTextIndexAt(Point<int> const pt) const
{
    return getTextIndexAt(pt.x, pt.y);
}

int UnicodeTextEditor::getCharIndexForPoint(Point<int> const point) const
{
    return getTextIndexAt(isMultiLine() ? point : getTextBounds({ 0, getTotalNumChars() }).getBounds().getConstrainedPoint(point));
}

void UnicodeTextEditor::insertTextAtCaret(String const& t)
{
    ...
    int const insertIndex = selection.getStart();
    int const newCaretPos = insertIndex + newText.length();
...
void UnicodeTextEditor::setHighlightedRegion(Range<int> const& newSelection)
{
    if (newSelection == getHighlightedRegion())
        return;

    auto const cursorAtStart = newSelection.getEnd() == getHighlightedRegion().getStart()
...
//==============================================================================
void UnicodeTextEditor::addPopupMenuItems(PopupMenu& m, MouseEvent const*)
{
    bool const writable = !isReadOnly();
...
void UnicodeTextEditor::performPopupMenuAction(int const menuItemID)
{
    switch (menuItemID) {
...
//==============================================================================
void UnicodeTextEditor::mouseDown(MouseEvent const& e)
{
...
void UnicodeTextEditor::mouseDrag(MouseEvent const& e)
{
    if (!mouseDownInEditor)
        return;
...
void UnicodeTextEditor::mouseUp(MouseEvent const& e)
{
    if (!mouseDownInEditor)
        return;
...
void UnicodeTextEditor::mouseDoubleClick(MouseEvent const& e)
{
    if (!mouseDownInEditor)
        return;
...
void UnicodeTextEditor::mouseWheelMove(MouseEvent const& e, MouseWheelDetails const& wheel)
{
    if (!mouseDownInEditor)
        return;
...
//==============================================================================
bool UnicodeTextEditor::moveCaretWithTransaction(int const newPos, bool const selecting)
{
    newTransaction();
...
bool UnicodeTextEditor::moveCaretUp(bool selecting)
{
    ...
    auto const caretPos = (getCaretRectangle() - getTextOffset()).toFloat();
...
bool UnicodeTextEditor::keyPressed(KeyPress const& key)
{
...
bool UnicodeTextEditor::keyStateChanged(bool const isKeyDown)
{
    if (!isKeyDown)
        return false;
...
void UnicodeTextEditor::handleCommandMessage(int const commandId)
{
...
void UnicodeTextEditor::setTemporaryUnderlining(Array<Range<int>> const& newUnderlinedSections)
{
...
void UnicodeTextEditor::insert(String const& text, int insertIndex, Font const& font,
...
void UnicodeTextEditor::reinsert(int insertIndex, OwnedArray<UniformTextSection> const& sectionsToInsert)
{
    int index = 0;
    int nextIndex = 0;
...
void UnicodeTextEditor::remove(Range<int> range, UndoManager* const um, int const caretPositionToMoveTo)
{
    if (!range.isEmpty()) {
        int index = 0;
...
String UnicodeTextEditor::getTextInRange(Range<int> const& range) const
{
    if (range.isEmpty())
        return {};
...
int UnicodeTextEditor::indexAtPosition(float const x, float const y) const
{
    if (getWordWrapWidth() > 0) {
...
//==============================================================================
int UnicodeTextEditor::findWordBreakAfter(int const position) const
{
...
int UnicodeTextEditor::findWordBreakBefore(int const position) const
{
    if (position <= 0)
        return 0;
...
//==============================================================================
void UnicodeTextEditor::splitSection(int const sectionIndex, int const charToSplitAt)
{
...
private:
    ...
    public:
        ...
        void setText(String const& newText) override
        {
        ...
        RectangleList<int> getTextBounds(Range<int> textRange) const override
        {
            ...
            std::for_each(localRects.begin(), localRects.end(),
                [&](Rectangle<int> const& r) { globalRects.add(unicodeTextEditor.localAreaToGlobal(r)); });
...
UnicodeLabel::UnicodeLabel(String const& name, String const& labelText)
    : Component(name)
    , textValue(labelText)
...
//==============================================================================
void UnicodeLabel::setText(String const& newText, NotificationType notification)
{
    hideEditor(true);
...
//==============================================================================
void UnicodeLabel::setFont(Font const& newFont)
{
    if (font != newFont) {
        font = newFont;
        repaint();
    ...
    bool lossOfFocusDiscards)
{
    ...
    auto const isKeybordFocusable = (editOnSingleClick || editOnDoubleClick);
...
void UnicodeLabel::hideEditor(bool discardCurrentEditorContents)
{
    if (editor != nullptr) {
        ...
        bool const changed = (!discardCurrentEditorContents)
...
void UnicodeLabel::mouseUp(MouseEvent const& e)
{
    if (editSingleClick
        && isEnabled()
...
void UnicodeLabel::mouseDoubleClick(MouseEvent const& e)
{
    if (editDoubleClick
        && isEnabled()
        && !e.mods.isPopupMenu()) {
...
void UnicodeLabel::setMinimumHorizontalScale(float const newScale)
{
...
public:
    explicit UnicodeLabelKeyboardFocusTraverser(UnicodeLabel& l)
        : owner(l)
    {
    }
...
private:
    ...
    public:
        ...
        void setValueAsString(String const&) override { }
