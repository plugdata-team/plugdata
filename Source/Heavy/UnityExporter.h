...
class UnityExporter final : public ExporterBase {
public:
    UnityExporter(PluginEditor* editor, ExportingProgressView* exportingView)
    ...
    void getState(DynamicObject::Ptr globalState) override
    {
        ...
        globalState->setProperty("unity", state);
    }

    void setState(DynamicObject::Ptr globalState) override
    {
        auto const state = globalState->getProperty("unity").getDynamicObject();
        if (!state)
            return;
