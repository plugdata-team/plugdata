...
struct NumboxComponent : public GUIComponent {
    ...
    void mouseDrag(const MouseEvent &e) override {
        ...
        if (!input.isBeingEdited()) {
            ...
            auto precision = static_cast<int>(std::lower_bound(xOffsets.begin(), xOffsets.end(), position) -
                                             xOffsets.begin());
    ...
    ObjectParameters defineParamters() override {
        ...
        return {{{"Minimum", tFloat, static_cast<void*>(&min)},
...
struct VUMeter : public GUIComponent {
    ...
    void paint(Graphics &g) override {
        ...
        g.drawFittedText(String(rms, 2) + " dB", Rectangle<int>(getLocalBounds().removeFromBottom(20)).reduced(2),
