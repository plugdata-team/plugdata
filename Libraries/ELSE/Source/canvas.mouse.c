...
static void canvas_mouse_proxy_any(t_canvas_mouse_proxy *p, t_symbol*s, int ac, t_atom *av){
    if(p->p_parent && ac){
        ...
        if(s == gensym("motion") && (p->p_parent->x_enable_edit_mode || !p->p_parent->x_edit)){
            outlet_float(p->p_parent->x_outlet_y, (float)y);
            outlet_float(p->p_parent->x_outlet_x, (float)x);
        }
        else if(s == gensym("mouse") && (p->p_parent->x_enable_edit_mode || !p->p_parent->x_edit)){
        ...
        else if(s == gensym("mouseup") && (p->p_parent->x_enable_edit_mode || !p->p_parent->x_edit)){
            if((av+2)->a_w.w_float)
...
static void *canvas_mouse_new(t_floatarg f1, t_floatarg f2, t_floatarg f3){
...
void setup_canvas0x2emouse(void){
    canvas_mouse_class = class_new(gensym("canvas.mouse"), (t_newmethod)canvas_mouse_new,
        (t_method)canvas_mouse_free, sizeof(t_canvas_mouse), 0, A_DEFFLOAT, A_DEFFLOAT, A_DEFFLOAT, 0);
