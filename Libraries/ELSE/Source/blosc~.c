...
static void blosc_dsp(t_blosc *x, t_signal **sp)
{
    ...
    /* 2 osc */
    if (x->x_ctl.c_waveshape == gensym("syncsaw")){
        x->x_ctl.c_scale = 1.0;
        x->x_ctl.c_scale_update = 1.0;
    ...
    /* 1 osc */
    else if (x->x_ctl.c_waveshape == gensym("pulse")){
        x->x_ctl.c_scale = 1.0;
        x->x_ctl.c_scale_update = 1.0;
        dsp_add(blosc_perform_pulse, 4, &x->x_ctl, sp[0]->s_n, sp[0]->s_vec, sp[1]->s_vec);
    }
    else if (x->x_ctl.c_waveshape == gensym("pulse2")){
        x->x_ctl.c_phase_inc_scale *= 2;
        x->x_ctl.c_scale = 1.0;
        x->x_ctl.c_scale_update = -1.0;
        dsp_add(blosc_perform_pulse, 4, &x->x_ctl, sp[0]->s_n, sp[0]->s_vec, sp[1]->s_vec);
    }
    else if (x->x_ctl.c_waveshape == gensym("comparator")) {
        x->x_ctl.c_scale = 1.0;
        x->x_ctl.c_scale_update = 1.0;
