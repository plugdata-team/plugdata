...
static t_int *saw2_perform_sig(t_int *w)
{
    t_saw2 *x = (t_saw2 *)(w[1]);
    int nblock = (t_int)(w[2]);
    t_float *in1 = (t_float *)(w[3]); // freq
...
static void saw2_dsp(t_saw2 *x, t_signal **sp){
    ...
    if (x->x_hasfeeders){
        ...
        dsp_add(saw2_perform, 6, x, sp[0]->s_n, sp[0]->s_vec, sp[1]->s_vec, sp[2]->s_vec, sp[3]->s_vec);
    }
}
...
void saw2_tilde_setup(void){
    saw2_class = class_new(gensym("saw2~"),
        (t_newmethod)saw2_new, (t_method)saw2_free,
        sizeof(t_saw2), CLASS_DEFAULT, A_GIMME, 0);
    CLASS_MAINSIGNALIN(saw2_class, t_saw2, x_freq);
