...
static void cyclone_mifiread_earlyeof(t_mifiread *mr)
{
    mr->mr_bytesleft = 0;
    mr->mr_eof = 1;
}
...
static uchar cyclone_mifiread_getbyte(t_mifiread *mr){
    if(mr->mr_bytesleft){
        int c;
        if((c = fgetc(mr->mr_fp)) == EOF){
            cyclone_mifiread_earlyeof(mr);
            return(0);
        }
        else{
            mr->mr_bytesleft--;
...
static uint32 cyclone_mifiread_getbytes(t_mifiread *mr, uchar *buf, uint32 size){
    size_t res;
...
static int cyclone_mifiread_skipbytes(t_mifiread *mr, uint32 size){
    if(size > mr->mr_bytesleft)
        size = mr->mr_bytesleft;
    if(size){
...
static uint32 cyclone_mifiread_getvarlen(t_mifiread *mr){
    uint32 n = 0;
    uchar c;
    ...
    if(count > 4)
        count = 4;
    while(count--){
        n = (n << 7) + ((c = cyclone_mifiread_getbyte(mr)) & 0x7f);
        if((c & 0x80) == 0)
            break;
    }
    return(n);
...
static size_t cyclone_mifiwrite_putvarlen(t_mifiwrite *mw, uint32 n){
    uint32 buf = n & 0x7f;
    size_t length = 1;
    while((n >>= 7) > 0){
        buf <<= 8;
...
static void cyclone_mifiread_updateticks(t_mifiread *mr){
    if(mr->mr_nframes){
        ...
        if(mr->mr_ticks.rt_tempo < MIFI_TICKEPSILON){
            post("bug: cyclone_mifiread_updateticks");
            mr->mr_ticks.rt_tempo = mr->mr_ticks.rt_deftempo;
        }
    }
...
static void cyclone_mifiread_resetticks(t_mifiread *mr){
...
static void cyclone_mifiread_reset(t_mifiread *mr){
    mr->mr_eof = 0;
    mr->mr_newtrack = 0;
    mr->mr_fp = 0;
    mr->mr_format = 0;
    ...
    mr->mr_hdtracks = 1;
    mr->mr_tracknames = 0;
    cyclone_mifiread_updateticks(mr);
}
...
static int cyclone_mifiread_startfile(t_mifiread *mr, const char *filename,
const char *dirname, int complain){
    ...
    if(fread(&mr->mr_header, 1, MIFIHARD_HEADERSIZE, mr->mr_fp) < MIFIHARD_HEADERSIZE){
...
static int cyclone_mifiread_starttrack(t_mifiread *mr){
    t_mifitrackheader th;
    long skip;
    int notyet = 1;
    do{
        if(fread(&th, 1, MIFIHARD_TRACKHEADERSIZE, mr->mr_fp) < MIFIHARD_TRACKHEADERSIZE)
            goto nomoretracks;
        ...
        if(strncmp(th.th_type, "MTrk", 4)){
            ...
            buf[4] = 0;
            if(mr->mr_pass == 1)
                mifi_warning(mr->mr_owner, "unknown chunk %s in midi file... skipped", buf);
        }
        else if(th.th_length < MIFI_SHORTESTEVENT){
            if(mr->mr_pass == 1)
                mifi_warning(mr->mr_owner, "empty track in midi file... skipped");
        }
        else
            notyet = 0;
...
static int cyclone_mifiread_nextevent(t_mifiread *mr){
    t_mifievent *ep = &mr->mr_event;
    uchar status;
    uint32 length;
    mr->mr_newtrack = 0;
nextattempt:
    if(mr->mr_bytesleft < MIFI_SHORTESTEVENT && !cyclone_mifiread_starttrack(mr))
        return(MIFIREAD_EOF);
    mr->mr_scoretime += (mr->mr_event.e_delay = cyclone_mifiread_getvarlen(mr));
    if((status = cyclone_mifiread_getbyte(mr)) < 0x80){
        if(MIFI_ISCHANNEL(mr->mr_status)){
            ...
            if(mr->mr_pass == 1)
                mifi_warning(mr->mr_owner, "missing running status in midi file... skip to end of track");
            goto endoftrack;
        }
    }
    else
    ...
    if(status < 0xf0){
        if(ep->e_length == 0){
            ep->e_data[0] = cyclone_mifiread_getbyte(mr);
            ep->e_length = 1;
        ...
        if(!MIFI_ONEDATABYTE(status)){
            ep->e_data[1] = cyclone_mifiread_getbyte(mr);
            ep->e_length = 2;
        }
    }
    ...
    /* system exclusive */
        ...
        length = cyclone_mifiread_getvarlen(mr);
        if(length > MIFIEVENT_MAXSYSEX){  /* FIXME optional read */
            if(cyclone_mifiread_skipbytes(mr, length) < 0)
                return(MIFIREAD_FATAL);
        }
        else{
            uchar *tempbuf = getbytes(length);
            if(cyclone_mifiread_getbytes(mr, tempbuf, length) != length)
                return(MIFIREAD_FATAL);
#ifdef MIFI_DEBUG
    ...
    /* meta-event */
        ...
        ep->e_meta = cyclone_mifiread_getbyte(mr);
        length = cyclone_mifiread_getvarlen(mr);
        if(ep->e_meta > 127){
        ...
        switch (ep->e_meta){
            ...
            case MIFIMETA_TEMPO:
                ...
                if(cyclone_mifiread_getbytes(mr, ep->e_data + 1, 3) != 3)
                    return(MIFIREAD_FATAL);
                ep->e_data[0] = 0;
            ...
            case MIFIMETA_TIMESIG:
                ...
                if(cyclone_mifiread_getbytes(mr, ep->e_data, 4) != 4)
                    return(MIFIREAD_FATAL);
...
/* #ifdef MIFI_DEBUG
		...
		loudbug_post("barspan (hard) %g", mr->mr_ticks.rt_hardbar);
                ...
                if(cyclone_mifiread_getbytes(mr, ep->e_data, length) != length)
                    return(MIFIREAD_FATAL);
                ep->e_length = length;
...
endoftrack:
    if(cyclone_mifiread_skipbytes(mr, mr->mr_bytesleft) < 0)
        return(MIFIREAD_FATAL);
    else
        return(MIFIREAD_SKIP);
}

static int cyclone_mifiread_restart(t_mifiread *mr, int complain){
    mr->mr_eof = 0;
    mr->mr_newtrack = 0;
    mr->mr_status = 0;
    mr->mr_channel = 0;
...
static int cyclone_mifiread_doopen(t_mifiread *mr, const char *filename, const char *dirname, int complain){
    long skip;
    cyclone_mifiread_reset(mr);
    if(!cyclone_mifiread_startfile(mr, filename, dirname, complain))
        return(0);
...
/* FIXME complaining */
static int cyclone_mifiread_analyse(t_mifiread *mr, int complain){
    t_mifievent *ep = &mr->mr_event;
    ...
    *tnamebuf = '\0';
    ...
    mr->mr_nevents = 0;
    mr->mr_ntempi = 0;
    while((evtype = cyclone_mifiread_nextevent(mr)) >= MIFIREAD_SKIP){
    ...
    while((evtype = cyclone_mifiread_nextevent(mr)) >= MIFIREAD_SKIP){
        ...
        if(isnewtrack && MIFI_ISCHANNEL(evtype)){
            ...
            if(ntracks > mr->mr_ntracks){
                post("bug: cyclone_mifiread_doit: too many tracks");
                goto doitfail;
            }
            if(!mr->mr_tracknames[mr->mr_trackndx] || mr->mr_tracknames[mr->mr_trackndx] == &s_){
                post("bug: cyclone_mifiread_doit: empty track name");
...
/* cyclone_mifiread_get... calls to be used in the main read routine */
...
/* cyclone_mifiread_get... calls to be used in a mifireadhook */
...
double cyclone_mifiread_gettick(t_mifiread *mr){
    return(mr->mr_ticks.rt_tickscoef * (mr->mr_scoretime % (int)mr->mr_ticks.rt_hardbar));
}
...
t_symbol *cyclone_mifiread_gettrackname(t_mifiread *mr){
    if(mr->mr_pass == 2 && mr->mr_tracknames && mr->mr_trackndx < mr->mr_ntracks)
        return(mr->mr_tracknames[mr->mr_trackndx]);
    else{
        post("bug: cyclone_mifiread_gettrackname");
        return(0);
    }
}
...
unsigned cyclone_mifiread_getchannel(t_mifiread *mr){
    if(mr->mr_pass != 2)
        post("bug: cyclone_mifiread_getchannel");
...
int cyclone_mifiread_open(t_mifiread *mr, const char *filename, const char *dirname, int complain){
    return(cyclone_mifiread_doopen(mr, filename, dirname, complain) &&
	    (cyclone_mifiread_analyse(mr, complain) == MIFIREAD_EOF) &&
	    cyclone_mifiread_restart(mr, complain));
}
...
void cyclone_mifiread_close(t_mifiread *mr){
    ...
    if(mr->mr_tracknames)
        freebytes(mr->mr_tracknames, mr->mr_hdtracks * sizeof(*mr->mr_tracknames));
}

void cyclone_mifiread_free(t_mifiread *mr){
    cyclone_mifiread_close(mr);
...
t_mifiread *cyclone_mifiread_new(t_pd *owner){
    ...
    cyclone_mifiread_resetticks(mr);
    cyclone_mifiread_reset(mr);
    return(mr);
}

static void cyclone_mifiwrite_updateticks(t_mifiwrite *mw){
    if(mw->mw_nframes){
        /* LATER ntsc */
        ...
        if(mw->mw_ticks.wt_tempo < MIFI_TICKEPSILON){
            post("bug: cyclone_mifiwrite_updateticks");
            mw->mw_ticks.wt_tempo = mw->mw_ticks.wt_deftempo;
        }
        mw->mw_ticks.wt_mscoef = (1000. * mw->mw_ticks.wt_beatticks) / mw->mw_tempo;
    }
}

static void cyclone_mifiwrite_resetticks(t_mifiwrite *mw){
...
static void cyclone_mifiwrite_reset(t_mifiwrite *mw){
    mw->mw_trackndx = 0;
    mw->mw_trackdirty = 0;
    mw->mw_fp = 0;
    ...
    mw->mw_channel = 0;
    mw->mw_trackbytes = 0;
    cyclone_mifiwrite_updateticks(mw);
}
...
static int cyclone_mifiwrite_putnextevent(t_mifiwrite *mw, t_mifievent *ep){
    uchar buf[3], *ptr = buf;
    size_t size = cyclone_mifiwrite_putvarlen(mw, ep->e_delay);
    if(!size)
        return(0);
    ...
    if(MIFI_ISCHANNEL(ep->e_status)){
        ...
        *ptr++ = ep->e_data[0];
        ...
        if(fwrite(buf, 1, 2, mw->mw_fp) != 2)
            return(0);
        mw->mw_trackbytes += 2;
        size = cyclone_mifiwrite_putvarlen(mw, ep->e_length);
        if(!size)
            return(0);
        mw->mw_trackbytes += size;
        size = ep->e_length;
...
int cyclone_mifiwrite_open(t_mifiwrite *mw, const char *filename, const char *dirname,
int ntracks, int complain){
    char errmess[MAXPDSTRING], fnamebuf[MAXPDSTRING];
    if(ntracks < 1 || ntracks > MIFI_MAXTRACKS){
        post("bug: cyclone_mifiwrite_open 1");
        complain = 0;
        goto wopenfailed;
    }
    mw->mw_ntracks = ntracks;
    cyclone_mifiwrite_reset(mw);
    if(mw->mw_format == 0){
        if(mw->mw_ntracks != 1){  /* LATER consider replacing with a warning */
            post("bug: cyclone_mifiwrite_open 2");
            complain = 0;
            goto wopenfailed;
        }
#ifdef MIFI_VERBOSE
        ...
        post("writing midi file \"%s\" (%d tracks)", filename, mw->mw_ntracks);
#endif
    ...
    if(fwrite(&mw->mw_header, 1, MIFIHARD_HEADERSIZE, mw->mw_fp) < MIFIHARD_HEADERSIZE){
...
static int cyclone_mifiwrite_adjusttrack(t_mifiwrite *mw, uint32 eotdelay, int complain){
    ...
    ep->e_length = 0;
    if(!cyclone_mifiwrite_putnextevent(mw, ep))
        return(0);
...
int cyclone_mifiwrite_opentrack(t_mifiwrite *mw, char *trackname, int complain){
    t_mifitrackheader th;
    if(mw->mw_trackdirty && !cyclone_mifiwrite_adjusttrack(mw, 0, complain))
        return(0);
    if(mw->mw_trackndx > mw->mw_ntracks)
        return(0);
    else if(mw->mw_trackndx++ == mw->mw_ntracks){
        post("bug: cyclone_mifiwrite_opentrack");
        return(0);
    }
    ...
    mw->mw_trackbytes = 0;
    if(fwrite(&th, 1, MIFIHARD_TRACKHEADERSIZE, mw->mw_fp) != MIFIHARD_TRACKHEADERSIZE){
        if(complain)
            mifi_error(mw->mw_owner, "unable to write midi file header (errno %d: %s)",
            errno, strerror(errno));
        return(0);
    }
    if(trackname){
        if(!cyclone_mifiwrite_textevent(mw, 0., MIFIMETA_TRACKNAME, trackname)){
            if(complain)
...
int cyclone_mifiwrite_closetrack(t_mifiwrite *mw, double enddelay, int complain){
    if(mw->mw_trackdirty){
        ...
        return(cyclone_mifiwrite_adjusttrack(mw, eotdelay, complain));
    }
    else{
        post("bug: cyclone_mifiwrite_closetrack");
        return(0);
    }
}

int cyclone_mifiwrite_textevent(t_mifiwrite *mw, double delay, unsigned type, char *text){
    t_mifievent *ep = &mw->mw_event;
    if(!mifievent_settext(ep, type, text))
        return(0);
    ep->e_delay = (uint32)(delay * mw->mw_ticks.wt_mscoef);
    return(cyclone_mifiwrite_putnextevent(mw, ep));
}
...
unsigned channel, unsigned data1, unsigned data2){
    ...
    if(!MIFI_ISCHANNEL(status) || channel > 15 || data1 > 127 || (!shorter && data2 > 127)){
        post("bug: cyclone_mifiwrite_channelevent");
        return(0);
    }
    ep->e_delay = (uint32)(delay * mw->mw_ticks.wt_mscoef);
...
void cyclone_mifiwrite_close(t_mifiwrite *mw){
    if(mw->mw_trackdirty)
        cyclone_mifiwrite_adjusttrack(mw, 0, 0);
    if(mw->mw_fp){
        fclose(mw->mw_fp);
        mw->mw_fp = 0;
    }
...
t_mifiwrite *cyclone_mifiwrite_new(t_pd *owner){
    ...
    cyclone_mifiwrite_resetticks(mw);
    cyclone_mifiwrite_reset(mw);
    return(mw);
}
