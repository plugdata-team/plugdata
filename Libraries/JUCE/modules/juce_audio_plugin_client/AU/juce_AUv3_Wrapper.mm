...
public:
    ...
    //==============================================================================
    virtual AUAudioUnitPreset* get_current_preset()                          = 0;
...
private:
    struct Class  : public ObjCClass<AUAudioUnit>
    {
        Class() : ObjCClass<AUAudioUnit> ("AUAudioUnit_")
        {
            ...
            //==============================================================================
            addMethod (@selector (currentPreset),                   get_current_preset,               "@@:");
        ...
        //==============================================================================
        static AUAudioUnitPreset* get_current_preset (id self, SEL)                                   { return _this (self)->get_current_preset(); }
...
public:
    ...
    //==============================================================================
    AUAudioUnitPreset* get_current_preset() override
    {
...
private:
    ...
    void addAudioUnitBusses (bool isInput)
    {
        ...
        const auto nuaudio_processorBuses = AudioUnitHelpers::getBusCount (processor, isInput);
        ...
        for (int i = 0; i < numWrapperBuses; ++i)
        {
            ...
            {
                const auto channels = nuaudio_processorBuses <= i ? 2 : processor.getChannelCountOfBus (isInput, i);
                                      ...
                                      AURenderPullInputBlock __nullable pullInputBlock)
    {
        ...
        if (lastTimeStamp.mSampleTime != timestamp->mSampleTime)
        {
            ...
            const auto nuaudio_processorBusesIn  = AudioUnitHelpers::getBusCount (processor, true);
            const auto nuaudio_processorBusesOut = AudioUnitHelpers::getBusCount (processor, false);
            ...
            // prepare buffers
            {
                for (int busIdx = 0; busIdx < numWrapperBusesOut; ++busIdx)
                {
                    ...
                    if (nuaudio_processorBusesOut <= busIdx)
            ...
            // pull inputs
            {
                for (int busIdx = 0; busIdx < nuaudio_processorBusesIn; ++busIdx)
                {
                    BusBuffer& busBuffer = *inBusBuffers[busIdx];
            ...
            // set buffer pointer to minimize copying
            {
                ...
                for (int busIdx = 0; busIdx < nuaudio_processorBusesOut; ++busIdx)
                {
                    BusBuffer& busBuffer = *outBusBuffers[busIdx];
                    AudioBufferList* buffer = busBuffer.get();
            ...
            // copy back
            if (outputBusNumber < nuaudio_processorBusesOut)
