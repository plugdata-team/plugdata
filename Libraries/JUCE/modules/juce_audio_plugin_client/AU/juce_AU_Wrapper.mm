...
private:
    ...
    {
        const auto nuaudio_processorBuses = AudioUnitHelpers::getBusCount (*juceFilter, false);
        ...
        for (UInt32 busIdx = 0; busIdx < numWrapperBuses; ++busIdx)
        {
            ...
            if (busIdx >= (UInt32) nuaudio_processorBuses)
    ...
    OSStatus syncAudioUnitWithProcessor()
    {
        ...
        const auto nuaudio_processorInputs  = AudioUnitHelpers::getBusCount (*juceFilter, true);
        const auto nuaudio_processorOutputs = AudioUnitHelpers::getBusCount (*juceFilter, false);

        for (int i = 0; i < nuaudio_processorInputs; ++i)
            if ((err = syncAudioUnitWithChannelSet (true,  i, juceFilter->getChannelLayoutOfBus (true,  i))) != noErr)
                return err;

        for (int i = 0; i < nuaudio_processorOutputs; ++i)
