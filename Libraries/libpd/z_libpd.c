...
static void* get_object(char const* s)
{
...
int libpd_init(void)
{
    ...
    if (s_initialized)
        return -1; // only allow init once (for now)
    s_initialized = 1;
...
void libpd_add_to_search_path(char const* path)
{
    sys_lock();
...
void* libpd_openfile(char const* name, char const* dir)
{
    void* retval;
    sys_lock();
    pd_globallock();
...
int libpd_process_short(int const ticks, short const* inBuffer, short* outBuffer)
{
    PROCESS(*short_to_sample, *sample_to_short)
}

int libpd_process_float(int const ticks, float const* inBuffer, float* outBuffer)
{
    PROCESS(, )
}

int libpd_process_double(int const ticks, double const* inBuffer, double* outBuffer)
{
    PROCESS(, )
}
...
int libpd_process_raw(float const* inBuffer, float* outBuffer)
{
    PROCESS_RAW(, )
}

int libpd_process_raw_short(short const* inBuffer, short* outBuffer)
{
    PROCESS_RAW(*short_to_sample, *sample_to_short)
}

int libpd_process_raw_double(double const* inBuffer, double* outBuffer)
{
    PROCESS_RAW(, )
}
...
int libpd_write_array(void* garray, int offset, float const* src, int n)
{
    sys_lock();
...
int libpd_write_array_double(void* garray, int offset, double const* src, int n)
{
    sys_lock();
    MEMCPY((vec++)->w_float, *src++)
    sys_unlock();
...
int libpd_bang(char const* recv)
{
    void* obj;
    sys_lock();
    obj = get_object(recv);
...
static int libpd_dofloat(char const* recv, t_float x)
{
    void* obj;
    sys_lock();
    obj = get_object(recv);
...
int libpd_float(char const* recv, float x)
{
    return libpd_dofloat(recv, x);
}

int libpd_double(char const* recv, double x)
{
    return libpd_dofloat(recv, x);
}

int libpd_symbol(char const* recv, char const* symbol)
{
    void* obj;
    sys_lock();
    obj = get_object(recv);
...
void libpd_add_symbol(char const* symbol)
{
    t_symbol* x;
    sys_lock();
    x = gensym(symbol);
...
int libpd_finish_list(char const* recv)
{
    return libpd_list(recv, s_argc, s_argv);
}

int libpd_finish_message(char const* recv, char const* msg)
{
...
void libpd_set_symbol(t_atom* a, char const* symbol)
{
    SETSYMBOL(a, gensym(symbol));
}

int libpd_list(char const* recv, int argc, t_atom* argv)
{
    t_pd* obj;
    sys_lock();
    obj = get_object(recv);
...
int libpd_message(char const* recv, char const* msg, int argc, t_atom* argv)
{
    t_pd* obj;
    sys_lock();
    obj = get_object(recv);
...
void* libpd_bind(char const* recv)
{
    t_symbol* x;
    sys_lock();
    x = gensym(recv);
...
int libpd_exists(char const* recv)
{
    int retval;
    sys_lock();
...
char const* libpd_get_symbol(t_atom* a)
{
...
int libpd_start_gui(char const* path)
{
    int retval;
    sys_lock();
...
void sys_loadpreferences(char const* filename, int startingup) { }
int sys_oktoloadfiles(int done) { return 1; }
void sys_savepreferences(char const* filename) { } // used in s_path.c
